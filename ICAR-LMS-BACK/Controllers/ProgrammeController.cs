...
namespace LMS.Controllers
{
    ...
    public class ProgrammeController : ControllerBase
    {
        ...
        public async Task<ActionResult<IEnumerable<Programme>>> GetAllProgrammes()
        {
            ...
            while (await reader.ReadAsync())
            {
                result.Add(new Programme
                {
                    ...
                    Credits = reader["Credits"] == DBNull.Value
            ? 0
                ...
                using (var conn = new SqlConnection(_connection))
                {
                    ...
                    using (var cmd = new SqlCommand("sp_Programme_CreateProgramme", conn))
                    {
                        ...
                        cmd.Parameters.AddWithValue("@Discipline",
                            string.IsNullOrWhiteSpace(programme.Discipline)
                                ? DBNull.Value
                                : programme.Discipline);

                        cmd.Parameters.AddWithValue("@Credits", programme.Credits);
        ...
        //        cmd.Parameters.AddWithValue("@Discipline",
        //string.IsNullOrWhiteSpace(updated.Discipline)
        //    ? DBNull.Value
        //    : updated.Discipline);

        //        cmd.Parameters.AddWithValue("@Credits", updated.Credits);
        ...
        public async Task<IActionResult> UpdateProgramme(int id, [FromBody] Programme updated)
        {
            try
            {
                ...
                cmd.Parameters.AddWithValue("@IsNoGrp", updated.IsNoGrp);
                ...
                using (SqlConnection conn = new SqlConnection(_connection))
                {
                    ...
                    foreach (var dto in batch)
                    {
                        using (SqlCommand cmd = new SqlCommand("Sp_UpsertBatches", conn))
                        {
                            ...
                            cmd.Parameters.AddWithValue("@Pid", dto.Pid);
                            cmd.Parameters.AddWithValue("@ExaminationId",dto.ExaminationId.HasValue
                            ? dto.ExaminationId.Value : (object)DBNull.Value);
                            cmd.Parameters.AddWithValue("@StartDate", dto.StartDate);
                            cmd.Parameters.AddWithValue("@EndDate", dto.EndDate);
