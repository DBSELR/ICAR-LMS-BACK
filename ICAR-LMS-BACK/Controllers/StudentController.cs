...
namespace LMS.Controllers
{
    ...
    public class StudentController : ControllerBase
    {
        ...
        [HttpGet("GetLandCourses")]
        [AllowAnonymous]
        public async Task<IActionResult> GetCoursesByBatch()
        {
            var result = new List<object>();
            using var conn = new SqlConnection(_configuration.GetConnectionString("DefaultConnection"));
            using var cmd = new SqlCommand("sp_GetCoursesByBatch", conn)
            {
                CommandType = CommandType.StoredProcedure
            };
        ...
        [HttpGet("LandingFeeCalculation")]
        [AllowAnonymous]
        public async Task<IActionResult> LandingFeeCalculation([FromQuery] string mobileNo = null)
        {
            var result = new List<object>();
            ...
            using var cmd = new SqlCommand(
                "SP_ProgrammeFeeDiscountCalculation",
                conn
            )
            {
                CommandType = CommandType.StoredProcedure
            ...
            cmd.Parameters.Add("@MobileNo", SqlDbType.NVarChar, 15)
                .Value = string.IsNullOrWhiteSpace(mobileNo)
                    ? DBNull.Value
                    : mobileNo;
        ...
        //[HttpGet("LandingFeeCalculation")]
        //[AllowAnonymous]
        //public async Task<IActionResult> LandingFeeCalculation()
        //{
        //    var result = new List<object>();
        //    using var conn = new SqlConnection(_configuration.GetConnectionString("DefaultConnection"));
        //    using var cmd = new SqlCommand("SP_ProgrammeFeeDiscountCalculation", conn)
        //    {
        //        CommandType = CommandType.StoredProcedure
        //    };
        ...
        public async Task<IActionResult> GetCoupons()
        {
            ...
            using var cmd = new SqlCommand("sp_GetCoupons", con);
            cmd.CommandType = CommandType.StoredProcedure;

            await con.OpenAsync();
            var rd = await cmd.ExecuteReaderAsync();
            while (await rd.ReadAsync())
                list.Add(rd["CouponCode"].ToString());
        ...
        [HttpGet("calculate")]
        [AllowAnonymous]
        public async Task<IActionResult> Calculate(decimal price, string coupon)
        {
            using var con = new SqlConnection(_configuration.GetConnectionString("DefaultConnection"));
            using var cmd = new SqlCommand("sp_CalculateCourseDiscount", con);
            cmd.CommandType = CommandType.StoredProcedure;

            cmd.Parameters.Add("@CoursePrice", SqlDbType.Decimal).Value = price;
            cmd.Parameters.Add("@CouponCode", SqlDbType.NVarChar).Value = coupon ?? "";
        ...
        [HttpPost("SaveAppliedDiscount")]
        [AllowAnonymous]
        public async Task<IActionResult> Save([FromBody] AppliedDiscountDto data)
        {
            using var con = new SqlConnection(_configuration.GetConnectionString("DefaultConnection"));
            using var cmd = new SqlCommand("sp_InsertAppliedDiscount", con);
            cmd.CommandType = CommandType.StoredProcedure;

            cmd.Parameters.AddWithValue("@StudentName", data.StudentName);
            cmd.Parameters.AddWithValue("@MobileNo", data.Mobile);
            cmd.Parameters.AddWithValue("@ProgrammeId", data.ProgrammeId);
            cmd.Parameters.AddWithValue("@CoursePrice", data.CoursePrice);
            cmd.Parameters.AddWithValue("@CouponCode", data.CouponCode);
            cmd.Parameters.AddWithValue("@DiscountPercent", data.DiscountPercent);
            cmd.Parameters.AddWithValue("@DiscountAmount", data.DiscountAmount);
            cmd.Parameters.AddWithValue("@FinalPrice", data.FinalPrice);

            // OUTPUT parameter
            var resultParam = new SqlParameter("@ResultMessage", System.Data.SqlDbType.NVarChar, 200)
            {
            ...
            var message = resultParam.Value.ToString();
            return Ok(new { message });
        }

        [HttpGet("GetAppliedDiscount/{mobile}")]
        [AllowAnonymous]
        public async Task<IActionResult> GetAppliedDiscount(string mobile)
        {
            using var conn = new SqlConnection(_configuration.GetConnectionString("DefaultConnection"));
            using var cmd = new SqlCommand("SELECT TOP 1 * FROM LMS_Applied_Discounts WHERE MobileNo=@Mobile ORDER BY CreatedOn DESC", conn);
            ...
            if (await reader.ReadAsync())
            {
                var discountData = new
                {
                    ...
                    FinalPrice = reader["FinalPrice"]
                };
                return Ok(discountData);
            }
            return Ok(null); // No previous discount
        }
        ...
        [HttpGet("GetStudentPaymentStatus/{mobile}")]
        [AllowAnonymous]
        public async Task<ActionResult<object>> GetStudentPaymentStatus(string mobile)
        {
            using var conn = new SqlConnection(_configuration.GetConnectionString("DefaultConnection"));
            using var cmd = new SqlCommand("sp_Student_GetPaymentStatus", conn) { CommandType = CommandType.StoredProcedure };
